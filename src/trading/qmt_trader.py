...
class TraderCallback:
    ...
    def on_stock_trade(self, trade):
        logger.info(f"成交回报: {trade.stock_code} 价{trade.traded_price} 量{trade.traded_volume}")

    def on_order_error(self, error):
        logger.error(f"委托报错: {error.error_msg}")

    def on_order_stock_async_response(self, response):
        logger.info(f"异步委托回调: seq={response.seq}")
...
class QMTTrader:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if result == 0:
                ...
                logger.info(f"交易连接成功, 账户: {self._account_id}")
                return True
            else:
                logger.error(f"交易连接失败: {result}")
                return False
        except ImportError:
            raise ImportError("xtquant未安装，无法使用交易功能")
        except Exception as e:
            logger.error(f"交易连接异常: {e}")
            return False
    ...
    def order_stock(
        ...
        logger.info(f"下单: {direction} {code} x{quantity} @ {price_type}, order_id={order_id}")
        return order_id
    ...
    def query_positions(self) -> List[Dict[str, Any]]:
        ...
        XtPosition 字段: stock_code, volume, can_use_volume, open_price,
        market_value, frozen_volume, on_road_volume, yesterday_volume, avg_price
        """
        if not self._connected:
            raise ConnectionError("未连接交易服务")
